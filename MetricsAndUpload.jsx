...
export function FileUploadPanel({ startProcessing, isProcessing, isComplete, fileStats, setFileStats }) {
  ...
  const handleFileChange = (e) => {
    ...
    const stats = { 
      ...
      docx: 0, 
      rejected: 0,
      uploadedList: files.map(f => {
        const ext = f.name.split('.').pop().toLowerCase();
        let isValid = ['pdf', 'docx', 'doc', 'xlsx', 'xls', 'csv'].includes(ext);
        return {
          name: f.name,
          size: f.size > 1024 * 1024 
            ? `${(f.size / (1024 * 1024)).toFixed(1)} MB` 
            : `${Math.round(f.size / 1024)} KB`,
          type: ext === 'doc' ? 'docx' : (ext === 'xls' ? 'xlsx' : ext),
          valid: isValid,
          reason: isValid 
            ? `Loss Run Detection Agent analyzed layout elements. Verified ${ext.toUpperCase()} document.`
            : `Classified as non-loss run and excluded.`
        };
      })
    };
    files.forEach(f => {
